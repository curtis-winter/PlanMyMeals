...
import { Settings, Moon, Sun, Globe, Loader2, CheckCircle2, AlertCircle, Cpu, Sparkles, Plus, X, Server, Trash2, ChevronDown } from 'lucide-react';
...
type SectionKey = 'userPreferences' | 'recipeGeneration' | 'recipeImport' | 'recipeCleanup' | 'aiSettings' | 'about';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [sections, setSections] = useState<Record<SectionKey, boolean>>(sectionDefaults);
  ...
  const SectionHeader: React.FC<{ title: string; sectionKey: SectionKey }> = ({ title, sectionKey }) => (
    <button 
      ...
      className="w-full flex items-center justify-between text-sm font-black text-primary uppercase tracking-widest border-b border-border-theme pb-2 hover:text-secondary"
    >
      {title}
      <ChevronDown className={`w-4 h-4 transition-transform ${sections[sectionKey] ? '' : '-rotate-90'}`} />
    </button>
  );
    ...
    >
      <div className="p-6 space-y-8">
        <section className="space-y-4">
          <SectionHeader title="User Preferences" sectionKey="userPreferences" />
        ...
        <section className="space-y-4">
          <SectionHeader title="Recipe Generation" sectionKey="recipeGeneration" />
          {sections.recipeGeneration && (
            <div className="space-y-4">
              <div>
        ...
        <section className="space-y-4">
          <SectionHeader title="Recipe Import" sectionKey="recipeImport" />
          {sections.recipeImport && (
            <div>
              <div>
                <label className="block text-xs font-bold text-neutral-theme uppercase tracking-wider mb-2 flex items-center gap-2">
        ...
        <section className="space-y-4">
          <SectionHeader title="Recipe Cleanup" sectionKey="recipeCleanup" />
          {sections.recipeCleanup && (
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-neutral-theme uppercase tracking-wider mb-2 flex items-center gap-2">
        ...
        <section className="space-y-4">
          <SectionHeader title="AI Settings" sectionKey="aiSettings" />
          {sections.aiSettings && (
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-neutral-theme uppercase tracking-wider mb-2 flex items-center gap-2">
        ...
        <section className="space-y-4">
          <SectionHeader title="About" sectionKey="about" />
          {sections.about && (
            <div>
              <label className="block text-xs font-bold text-neutral-theme uppercase tracking-wider mb-2 flex items-center gap-2">
                <Cpu className="w-3 h-3" /> Build #: {buildNumber || '—'}
              </label>
            </div>
          )}
        </section>
